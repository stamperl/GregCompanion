...
export const steelTankCapacityMs = machineSteamCapacityLitres('steelTank') * steamMsPerLitre
...
export const steelTankFluidCapacityLitres = machineFluidCapacityLitres('steelTank')
...
export const liquidSteamBoilerCreosoteUseLitresPerSecond = 3
...
function cloneProcessState(process: MachineProcessState): MachineProcessState {
  return {
    ...
    fluidFlowFluidId: process.fluidFlowFluidId,
  }
}
...
function normalizeResourceAmounts(parsed: unknown): ResourceAmount[] {
  if (!Array.isArray(parsed)) return []
  return combineResourceAmounts(parsed.flatMap((entry): ResourceAmount[] => {
    if (!entry || typeof entry !== 'object') return []
    const candidate = entry as Partial<ResourceAmount>
    if (!candidate.id || !(candidate.id in resourceRegistry)) return []
    const amount = Math.max(0, Math.floor(Number(candidate.amount) || 0))
    return amount > 0 ? [{ id: candidate.id, amount }] : []
  }))
}

function normalizeFluidAmounts(parsed: unknown) {
  if (!Array.isArray(parsed)) return []
  return parsed.flatMap((entry): import('./types').FluidAmount[] => {
    if (!entry || typeof entry !== 'object') return []
    const candidate = entry as Partial<import('./types').FluidAmount>
    if (!candidate.id || !fluidIds.includes(candidate.id)) return []
    const amount = normalizeLitres(Number(candidate.amount) || 0)
    return amount > 0 ? [{ id: candidate.id, amount, bufferId: typeof candidate.bufferId === 'string' ? candidate.bufferId : undefined }] : []
  })
}
...
function normalizeRecipeCards(parsed: unknown, machineInstances: MachineInstance[]): RecipeCardInstance[] {
  if (!Array.isArray(parsed)) return []
  const seen = new Set<string>()
  return parsed.flatMap((entry): RecipeCardInstance[] => {
    if (!entry || typeof entry !== 'object') return []
    ...
    return [{
      uid: candidate.uid,
      kind: candidate.kind,
      name: String(candidate.name || 'Encoded Recipe'),
      recipeId: typeof candidate.recipeId === 'string' ? candidate.recipeId : undefined,
      targetMachineId: candidate.targetMachineId && candidate.targetMachineId in machines ? candidate.targetMachineId : undefined,
      programNumber: Math.max(0, Math.min(10, Math.floor(Number(candidate.programNumber) || 0))),
      itemInputs: normalizeResourceAmounts(candidate.itemInputs),
      fluidInputs: normalizeFluidAmounts(candidate.fluidInputs),
      itemOutputs: normalizeResourceAmounts(candidate.itemOutputs),
      fluidOutputs: normalizeFluidAmounts(candidate.fluidOutputs),
      installedInUid,
    }]
  })
}
...
function normalizeFabricationJobs(parsed: unknown, cards: RecipeCardInstance[], machineInstances: MachineInstance[]): FabricationJob[] {
  ...
  return parsed.flatMap((entry): FabricationJob[] => {
    ...
    const card = cards.find((item) => item.uid === candidate.cardUid)
    if (!candidate.uid || seen.has(candidate.uid) || !card) return []
    seen.add(candidate.uid)
    const status = candidate.status === 'complete' || candidate.status === 'cancelled' || candidate.status === 'blocked' || candidate.status === 'running'
      ? candidate.status
      : 'queued'
    return [{
      uid: candidate.uid,
      cardUid: card.uid,
      requestedOutput: normalizeResourceAmounts(candidate.requestedOutput ? [candidate.requestedOutput] : [])[0] ?? card.itemOutputs[0] ?? { id: 'cobblestone', amount: 1 },
      batches: Math.max(1, Math.floor(Number(candidate.batches) || 1)),
      ...
      controllerUid: candidate.controllerUid && machineInstances.some((instance) => instance.uid === candidate.controllerUid && instance.machineId === 'planningController') ? candidate.controllerUid : undefined,
      interfaceUid: candidate.interfaceUid && interfaceUids.has(candidate.interfaceUid) ? candidate.interfaceUid : undefined,
      reservedItems: normalizeResourceAmounts(candidate.reservedItems),
      reservedFluids: normalizeFluidAmounts(candidate.reservedFluids),
      steps: Array.isArray(candidate.steps) ? candidate.steps.flatMap((step, index) => {
        if (!step || typeof step !== 'object') return []
        const parsedStep = step as Partial<FabricationJob['steps'][number]>
        if (!parsedStep.cardUid || !cards.some((item) => item.uid === parsedStep.cardUid)) return []
        return [{
          uid: String(parsedStep.uid || `step-${index + 1}`),
          cardUid: parsedStep.cardUid,
          ...
          interfaceUid: typeof parsedStep.interfaceUid === 'string' && interfaceUids.has(parsedStep.interfaceUid) ? parsedStep.interfaceUid : undefined,
          targetUid: typeof parsedStep.targetUid === 'string' ? parsedStep.targetUid : undefined,
          dispatched: Boolean(parsedStep.dispatched),
        }]
      }) : [],
      progressMs: Math.max(0, Math.floor(Number(candidate.progressMs) || 0)),
...
function normalizeProcessState(process?: Partial<MachineProcessState>): MachineProcessState {
  ...
  return {
    ...
    fluidFlowFluidId: process.fluidFlowFluidId && fluidIds.includes(process.fluidFlowFluidId) ? process.fluidFlowFluidId : undefined,
  }
}
...
function normalizeMachineInstances(
  ...
  const migratedInstances = instances.map((instance) => (
    (instance as { machineId?: string }).machineId === 'rackFrame'
      ? { ...instance, machineId: 'memoryModule' as MachineId }
      : instance
  ))
  for (const instance of migratedInstances
    ...
    normalized.push({
      ...
      fabricationCableInstalled: machineId === 'fabricationCable' || Boolean(instance.fabricationCableInstalled),
      fabricationInterfaces,
      ...
      installedRecipeCardUids: Array.isArray(instance.installedRecipeCardUids)
        ? instance.installedRecipeCardUids.filter((uid): uid is string => typeof uid === 'string').slice(0, 9)
        : [],
      fabricationPriority: Math.max(-10, Math.min(10, Math.floor(Number(instance.fabricationPriority) || 0))),
      fabricationFace: pipeDirections.includes(instance.fabricationFace as PipeDirection)
        ? instance.fabricationFace as PipeDirection
        : undefined,
      surveyCardTarget:
...
export function cloneState(state: GameState): GameState {
  return {
    ...
    recipeCards: (state.recipeCards ?? []).map((card) => ({
      ...card,
      itemInputs: card.itemInputs.map((amount) => ({ ...amount })),
      fluidInputs: card.fluidInputs.map((amount) => ({ ...amount })),
      itemOutputs: card.itemOutputs.map((amount) => ({ ...amount })),
      fluidOutputs: card.fluidOutputs.map((amount) => ({ ...amount })),
    })),
    fabricationJobs: (state.fabricationJobs ?? []).map((job) => ({
      ...job,
      requestedOutput: { ...job.requestedOutput },
      reservedItems: job.reservedItems.map((amount) => ({ ...amount })),
      reservedFluids: job.reservedFluids.map((amount) => ({ ...amount })),
  ...
  if (recipe.pattern) {
    ...
    recipe.pattern.slice(0, 9).forEach((patternSlot, index) => {
      ...
      const amount = occurrence === (patternCounts[id] ?? 1)
        ? Math.max(1, (declared[id] ?? 1) - occurrence + 1)
        : 1
      const candidates = recipeCatalystIds(recipe).has(id) ? resourceCostCandidates(id) : [id]
      const displayId = remaining ? (candidates.find((candidate) => remaining[candidate] >= amount) ?? id) : id
      const ghost = remaining ? candidates.every((candidate) => remaining[candidate] < amount) : false
      slots[index] = { id: displayId, ghost, ...(amount > 1 ? { amount } : {}) }
      if (remaining && !ghost) remaining[displayId] -= amount
    })
    return slots
  }
...
function nextFabricationUid(prefix: string, existing: string[]) {
  let index = existing.length + 1
  while (existing.includes(`${prefix}-${index}`)) index += 1
  return `${prefix}-${index}`
}

function cardItemOutputs(card: RecipeCardInstance) {
...
export function encodeCraftingRecipeCard(state: GameState, encoderUid: string, recipeId: string) {
  ...
  const recipe = recipes.find((candidate) => candidate.id === recipeId && candidate.outputs.length > 0)
  ...
  nextController.process.euStored -= 64
  const uid = nextFabricationUid('card', next.recipeCards.map((card) => card.uid))
  next.recipeCards.push({
    uid,
    kind: 'crafting',
    name: recipe.name,
    recipeId: recipe.id,
    programNumber: 0,
    itemInputs: combineResourceAmounts(recipe.inputs),
    fluidInputs: recipe.fluidInputs?.map((amount) => ({ ...amount })) ?? [],
    itemOutputs: combineResourceAmounts(recipe.outputs),
    fluidOutputs: recipe.fluidOutputs?.map((amount) => ({ ...amount })) ?? [],
  })
  next.recipeMilestones.fabrication_card_encoded = (next.recipeMilestones.fabrication_card_encoded ?? 0) + 1
  return next
}
...
export function encodeProcessingRecipeCard(state: GameState, encoderUid: string, recipeId: string) {
  ...
  next.recipeCards.push({
    ...
    name: recipe.name,
    recipeId: recipe.id,
    targetMachineId: recipe.machineId,
    programNumber: recipe.programNumber ?? 0,
    itemInputs: combineResourceAmounts([
      recipe.input,
      ...(recipe.secondaryInput ? [recipe.secondaryInput] : []),
      ...(recipe.extraInputs ?? []),
      ...(recipe.fuelInput ? [recipe.fuelInput] : []),
...
export function eraseRecipeCard(state: GameState, cardUid: string) {
  const card = state.recipeCards.find((candidate) => candidate.uid === cardUid)
  if (!card || card.installedInUid || state.fabricationJobs.some((job) => job.cardUid === cardUid && job.status !== 'complete' && job.status !== 'cancelled')) return state
  const next = addResources(state, [{ id: 'blankRecipeCard', amount: 1 }])
  next.recipeCards = next.recipeCards.filter((candidate) => candidate.uid !== cardUid)
  return next
}
...
export function attachFabricationFace(state: GameState, cableUid: string, direction: PipeDirection, machineId: FabricationFaceMachineId) {
  ...
  const next = cloneState(state)
  const nextCable = next.machineInstances.find((instance) => instance.uid === cableUid)!
...
export function removeRecipeCard(state: GameState, cardUid: string) {
  const card = state.recipeCards.find((candidate) => candidate.uid === cardUid)
...
export type PlanningRackStructure = {
  controller: MachineInstance
  originX: number
  originY: number
  width: number
  height: number
  cells: MachineInstance[]
  memoryUnits: number
  dispatchLanes: number
}

const planningRackModuleIds = new Set<MachineId>(['memoryModule', 'dispatchModule'])
const planningRackFootprints = [
  { width: 3, height: 3 },
  { width: 3, height: 2 },
  { width: 2, height: 2 },
  { width: 1, height: 1 },
...
function planningRackBankHasLooseModules(state: GameState, originX: number, originY: number, width: number, height: number) {
  for (let y = originY - 1; y <= originY + height; y += 1) {
    for (let x = originX - 1; x <= originX + width; x += 1) {
      const besideHorizontalEdge = x >= originX && x < originX + width && (y === originY - 1 || y === originY + height)
      const besideVerticalEdge = y >= originY && y < originY + height && (x === originX - 1 || x === originX + width)
      if (!besideHorizontalEdge && !besideVerticalEdge) continue
      const neighbour = machineAt(state, x, y)
      if (neighbour && planningRackModuleIds.has(neighbour.machineId)) return true
    }
  }
  return false
}

export function planningRackStructureForInstance(state: GameState, controller: MachineInstance): PlanningRackStructure | null {
  if (controller.machineId !== 'planningController') return null
  for (const { width, height } of planningRackFootprints) {
    const cells: MachineInstance[] = []
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        const cell = machineAt(state, controller.x + x, controller.y + y)
        if (x === 0 && y === 0) {
          if (cell?.uid === controller.uid) cells.push(cell)
        } else if (cell && planningRackModuleIds.has(cell.machineId)) {
          cells.push(cell)
        }
      }
    }
    if (cells.length !== width * height) continue
    if (planningRackBankHasLooseModules(state, controller.x, controller.y, width, height)) continue
    return {
      controller,
      originX: controller.x,
      originY: controller.y,
      width,
      height,
      cells,
      memoryUnits: (1 + cells.filter((cell) => cell.machineId === 'memoryModule').length) * 64,
      dispatchLanes: Math.min(8, 1 + cells.filter((cell) => cell.machineId === 'dispatchModule').length),
    }
  }
  return null
}

export function planningRackStructureForPart(state: GameState, instance: MachineInstance): PlanningRackStructure | null {
  if (instance.machineId === 'planningController') return planningRackStructureForInstance(state, instance)
  if (!planningRackModuleIds.has(instance.machineId)) return null
  for (const controller of state.machineInstances.filter((candidate) => candidate.machineId === 'planningController')) {
    const structure = planningRackStructureForInstance(state, controller)
    if (structure?.cells.some((cell) => cell.uid === instance.uid)) return structure
  }
  return null
}

export function hasFabricationCable(instance: MachineInstance) {
...
function adjacentFabricationCables(state: GameState, instance: MachineInstance) {
  return adjacentPositions(state, instance.x, instance.y)
    .map((position) => machineAt(state, position.x, position.y))
    .filter((candidate): candidate is MachineInstance => Boolean(candidate && hasFabricationCable(candidate)))
}

export function fabricationNetworkForController(state: GameState, controllerUid: string): FabricationNetwork | null {
  const controller = state.machineInstances.find((instance) => instance.uid === controllerUid && instance.machineId === 'planningController')
  const rack = controller ? planningRackStructureForInstance(state, controller) : null
  if (!rack || !controller) return null
  const queue = adjacentFabricationCables(state, controller)
  const seen = new Set<string>()
  const cables: MachineInstance[] = []
  while (queue.length > 0) {
    const cable = queue.shift()!
    if (seen.has(cable.uid)) continue
    seen.add(cable.uid)
    cables.push(cable)
    for (const neighbour of adjacentFabricationCables(state, cable)) {
      if (!seen.has(neighbour.uid)) queue.push(neighbour)
    }
  }
  const devices = new Map<string, MachineInstance>()
  for (const cable of cables) {
    for (const position of adjacentPositions(state, cable.x, cable.y)) {
      const device = machineAt(state, position.x, position.y)
      if (!device || hasFabricationCable(device) || planningRackStructureForPart(state, device)) continue
...
export function fabricationNetworks(state: GameState) {
  return state.machineInstances
    .filter((instance) => instance.machineId === 'planningController')
    .map((controller) => fabricationNetworkForController(state, controller.uid))
    .filter((network): network is FabricationNetwork => Boolean(network))
}

export function fabricationNetworkForDevice(state: GameState, deviceUid: string) {
  return fabricationNetworks(state).find((network) => network.devices.some((device) => device.uid === deviceUid)) ?? null
}
...
function linkedTankControllers(state: GameState, network: FabricationNetwork) {
  const controllers = new Map<string, MachineInstance>()
  for (const link of network.devices.filter((instance) => instance.machineId === 'fluidStorageLink')) {
    for (const position of adjacentPositions(state, link.x, link.y)) {
      const tank = machineAt(state, position.x, position.y)
      if (!tank || !isTankStorageMachine(tank.machineId)) continue
      const storage = steamTankStorageForInstance(state, tank)
      controllers.set(storage.uid, storage)
    }
  }
  return [...controllers.values()].sort((a, b) => a.uid.localeCompare(b.uid))
}

function availableLinkedFluid(state: GameState, network: FabricationNetwork, fluidId: FluidId) {
  return linkedTankControllers(state, network).reduce((sum, tank) => sum + (tank.process.fluids[fluidId] ?? 0), 0)
}

function reserveLinkedFluid(state: GameState, network: FabricationNetwork, fluidId: FluidId, amount: number) {
  let remaining = amount
  for (const tank of linkedTankControllers(state, network).filter((candidate) => (candidate.process.fluids[fluidId] ?? 0) > 0)) {
    const moved = Math.min(remaining, tank.process.fluids[fluidId] ?? 0)
    tank.process.fluids[fluidId] = Math.max(0, (tank.process.fluids[fluidId] ?? 0) - moved)
    remaining -= moved
...
function returnLinkedFluid(state: GameState, network: FabricationNetwork, fluidId: FluidId, amount: number) {
  let remaining = amount
  const tanks = linkedTankControllers(state, network).sort((a, b) => {
    const aMatches = (a.process.fluids[fluidId] ?? 0) > 0 ? 0 : 1
    const bMatches = (b.process.fluids[fluidId] ?? 0) > 0 ? 0 : 1
    return aMatches - bMatches || a.uid.localeCompare(b.uid)
  })
  for (const tank of tanks) {
    const otherFluid = storedFluidTypes(tank.process).some((id) => id !== fluidId)
    if (otherFluid) continue
    const free = tank.process.fluidCapacityLitres - (tank.process.fluids[fluidId] ?? 0)
    const moved = Math.min(remaining, free)
    tank.process.fluids[fluidId] = (tank.process.fluids[fluidId] ?? 0) + moved
    remaining -= moved
    if (remaining <= 0) break
  }
  return remaining
...
function buildFabricationPlan(state: GameState, network: FabricationNetwork, rootCard: RecipeCardInstance, quantity: number): FabricationPlan | null {
  ...
  const installedCards = state.recipeCards
    ...
    .sort((a, b) => {
      const aPriority = interfacePriorities.get(a.installedInUid!) ?? 0
      const bPriority = interfacePriorities.get(b.installedInUid!) ?? 0
  ...
  const steps: FabricationJob['steps'] = []
  const visiting = new Set<string>()

  const planCard = (card: RecipeCardInstance, requiredOutput: number): boolean => {
    if (visiting.has(card.uid)) return false
    const primary = card.itemOutputs[0]
    if (!primary || primary.amount <= 0) return false
    visiting.add(card.uid)
    const batches = Math.ceil(requiredOutput / primary.amount)
    for (const input of card.itemInputs) {
      const needed = input.amount * batches
      const available = remaining.get(input.id) ?? 0
      const used = Math.min(available, needed)
      remaining.set(input.id, available - used)
      if (used > 0) itemNeeds.set(input.id, (itemNeeds.get(input.id) ?? 0) + used)
      const missing = needed - used
      if (missing <= 0) continue
      const producer = installedCards.find((candidate) => candidate.uid !== card.uid && candidate.itemOutputs.some((output) => output.id === input.id))
      if (producer) {
        if (!planCard(producer, missing)) return false
        const producedPerBatch = producer.itemOutputs.find((output) => output.id === input.id)?.amount ?? 0
        const produced = Math.ceil(missing / producedPerBatch) * producedPerBatch
        remaining.set(input.id, (remaining.get(input.id) ?? 0) + produced - missing)
      } else {
        itemNeeds.set(input.id, (itemNeeds.get(input.id) ?? 0) + missing)
      }
    }
    for (const input of card.fluidInputs) fluidNeeds.set(input.id, (fluidNeeds.get(input.id) ?? 0) + input.amount * batches)
    visiting.delete(card.uid)
    steps.push({
      uid: `step-${steps.length + 1}`,
      cardUid: card.uid,
      batches,
      completedBatches: 0,
  ...
  if (!planCard(rootCard, quantity)) return null
  return {
    steps,
    itemNeeds: [...itemNeeds].map(([id, amount]) => ({ id, amount })),
    fluidNeeds: [...fluidNeeds].map(([id, amount]) => ({ id, amount })),
  }
}
...
export function previewFabricationRequest(state: GameState, cardUid: string, quantity = 1): FabricationRequestPreview {
  ...
  const missingFluids = plan.fluidNeeds
    .map((amount) => ({ ...amount, amount: Math.max(0, amount.amount - availableLinkedFluid(state, network, amount.id)) }))
    .filter((amount) => amount.amount > 0)
  ...
  const rackBusy = state.fabricationJobs.some((job) => job.controllerUid === network.rack.controller.uid && (job.status === 'queued' || job.status === 'running' || job.status === 'blocked'))
  const rack = !rackBusy && network.rack.memoryUnits >= jobUnits ? network.rack : undefined
  const reason = missingItems.length > 0
    ...
    : missingFluids.length > 0
      ...
      : rackBusy
        ? 'This Planning Rack is already running a job.'
        : !rack
          ? 'This Planning Rack does not have enough memory.'
          : undefined
...
export function requestFabricationJob(state: GameState, cardUid: string, quantity = 1, now = Date.now()) {
  ...
  const network = fabricationNetworkForController(next, preview.rackUid)
  if (!network) return state
  for (const fluid of preview.fluidNeeds) {
    if (!reserveLinkedFluid(next, network, fluid.id, fluid.amount)) return state
  }
...
function removeReservedItems(job: FabricationJob, amounts: ResourceAmount[]) {
  for (const amount of amounts) {
    const stored = job.reservedItems.find((candidate) => candidate.id === amount.id)
    if (!stored || stored.amount < amount.amount) return false
  }
  for (const amount of amounts) {
    const stored = job.reservedItems.find((candidate) => candidate.id === amount.id)!
    stored.amount -= amount.amount
  }
  job.reservedItems = job.reservedItems.filter((amount) => amount.amount > 0)
  return true
}

function addReservedItems(job: FabricationJob, amounts: ResourceAmount[]) {
  job.reservedItems = combineResourceAmounts([...job.reservedItems, ...amounts])
}

function removeReservedFluids(job: FabricationJob, amounts: import('./types').FluidAmount[]) {
  for (const amount of amounts) {
    const stored = job.reservedFluids.find((candidate) => candidate.id === amount.id)
    if (!stored || stored.amount < amount.amount) return false
  }
  for (const amount of amounts) {
    const stored = job.reservedFluids.find((candidate) => candidate.id === amount.id)!
  ...
  job.reservedFluids = job.reservedFluids.filter((amount) => amount.amount > 0)
  return true
}

function addReservedFluids(job: FabricationJob, amounts: import('./types').FluidAmount[]) {
  for (const amount of amounts) {
    const stored = job.reservedFluids.find((candidate) => candidate.id === amount.id)
    if (stored) stored.amount += amount.amount
    else job.reservedFluids.push({ ...amount })
  }
}

export function cancelFabricationJob(state: GameState, jobUid: string) {
  const job = state.fabricationJobs.find((candidate) => candidate.uid === jobUid && candidate.status !== 'complete' && candidate.status !== 'cancelled')
  if (!job) return state
  let next = addResources(state, job.reservedItems)
  const nextJob = next.fabricationJobs.find((candidate) => candidate.uid === jobUid)!
  const network = nextJob.controllerUid ? fabricationNetworkForController(next, nextJob.controllerUid) : null
  for (const fluid of [...nextJob.reservedFluids]) {
    fluid.amount = network ? returnLinkedFluid(next, network, fluid.id, fluid.amount) : fluid.amount
  }
...
function canRecipeItemOutputsAccept(process: MachineProcessState, recipe: ProcessRecipe) {
  return recipeItemOutputs(recipe).every((produced, index) => {
    const slotId = processOutputSlotIds[index]
    return Boolean(slotId && canOutputAccept(process[slotId], produced))
  })
}

function addRecipeItemOutputs(process: MachineProcessState, recipe: ProcessRecipe) {
  recipeItemOutputs(recipe).forEach((output, index) => {
    const slotId = processOutputSlotIds[index]
    if (slotId) process[slotId] = addToProcessOutput(process[slotId], output)
  })
}
...
function steamTankStructureAtOrigin(
  state: GameState,
  originX: number,
  originY: number,
  spec: SteamTankStructureSpec,
  machineId: MachineId,
) {
  ...
  if (!controller || controller.machineId !== machineId || !isTankStorageMachine(controller.machineId) || controller.level !== area) return null

  const complete = positions.every((position) => {
    const candidate = machineAt(state, position.x, position.y)
    if (!candidate || candidate.machineId !== machineId) return false
...
export function steamTankStructureForInstance(state: GameState, instance: MachineInstance) {
  ...
  if (instance.level > 1) {
    ...
    return spec ? steamTankStructureAtOrigin(state, instance.x, instance.y, spec, instance.machineId) : null
  }
  ...
  for (const spec of steamTankStructureSpecs) {
    for (let dy = 0; dy < spec.height; dy += 1) {
      for (let dx = 0; dx < spec.width; dx += 1) {
        const structure = steamTankStructureAtOrigin(state, instance.x - dx, instance.y - dy, spec, instance.machineId)
...
export function steamTankCapacityMsForInstance(state: GameState, instance: MachineInstance) {
  if (!isTankStorageMachine(instance.machineId)) return 0
  const structure = steamTankStructureForInstance(state, instance)
  if (structure && structure.controller.uid !== instance.uid) return 0
  return machineSteamCapacityLitres(instance.machineId) * steamMsPerLitre * (structure?.area ?? 1)
}
...
export function steamTankFluidCapacityLitresForInstance(state: GameState, instance: MachineInstance) {
  ...
  const structure = steamTankStructureForInstance(state, instance)
  if (structure && structure.controller.uid !== instance.uid) return 0
  return machineFluidCapacityLitres(instance.machineId) * (structure?.area ?? 1)
}
  ...
  for (const spec of steamTankStructureSpecs) {
    ...
    for (let dy = 0; dy < spec.height; dy += 1) {
      for (let dx = 0; dx < spec.width; dx += 1) {
        ...
        if (!positions.every((position) => machineAt(state, position.x, position.y)?.machineId === placed.machineId)) continue
        ...
        const baseSteamCapacityMs = machineSteamCapacityLitres(placed.machineId) * steamMsPerLitre
        const baseFluidCapacityLitres = machineFluidCapacityLitres(placed.machineId)
        const steamStoredMs = Math.min(
          tanks.reduce((sum, tank) => sum + tank.process.steamStoredMs, 0),
          baseSteamCapacityMs * area,
        )
        ...
        const fluidCapacityLitres = baseFluidCapacityLitres * area
        ...
        controller.process.steamCapacityMs = baseSteamCapacityMs * area
...
function machineFluidCapacityForInstance(state: GameState, instance: MachineInstance) {
  if (isTankStorageMachine(instance.machineId)) return steamTankFluidCapacityLitresForInstance(state, instance)
...
function flowCellsForInstance(state: GameState, instance: MachineInstance) {
  ...
  const tankStructure = isTankStorageMachine(instance.machineId) ? steamTankStructureForInstance(state, instance) : null
  if (tankStructure) {
...
function fluidRouteBetween(state: GameState, source: MachineInstance, target: MachineInstance) {
  ...
  const queue = [...starts]
  for (const start of starts) previous.set(start.uid, null)
  let queueIndex = 0
  let reachedUid: string | null = null

  while (queueIndex < queue.length && !reachedUid) {
    const current = queue[queueIndex++]
    if (targetUids.has(current.uid) && !startUids.has(current.uid)) {
  ...
  if (!reachedUid) return null
  const route: MachineInstance[] = []
  let currentUid: string | null = reachedUid
  while (currentUid) {
    const instance = state.machineInstances.find((candidate) => candidate.uid === currentUid)
    if (instance && isSteamPipeMachine(instance.machineId)) route.push(instance)
    currentUid = previous.get(currentUid) ?? null
  }
  return route.reverse()
}

function isEuMultiblockBridge(state: GameState, instance: MachineInstance) {
  const controller = multiblockCenterForInstance(state, instance)
  return Boolean(controller && isEuNetworkMachine(controller.spec.controller))
}

function euRouteBetween(state: GameState, source: MachineInstance, target: MachineInstance) {
  const targetUids = new Set([target.uid])
  const previous = new Map<string, string | null>([[source.uid, null]])
  const queue = [source]
  let queueIndex = 0
  let reachedUid: string | null = null
  ...
  while (queueIndex < queue.length && !reachedUid) {
    const current = queue[queueIndex++]
    if (targetUids.has(current.uid) && current.uid !== source.uid) {
      reachedUid = current.uid
      break
    }
...
function fluidRouteAllowance(route: MachineInstance[], requested: number, elapsedMs: number, competingRoutes: MachineInstance[][] = [route]) {
  return route.reduce((allowance, pipe) => {
    const capacity = (machinePipeTransferLitresPerSecond(pipe.machineId) * elapsedMs) / 1000
    const available = consumeTickBudget(activeFluidTransferBudgets, `fluid-segment:${pipe.uid}`, capacity, capacity)
    const contenders = Math.max(1, competingRoutes.filter((candidate) => candidate.some((segment) => segment.uid === pipe.uid)).length)
    return Math.min(allowance, available / contenders)
  }, Math.max(0, requested))
}

function spendFluidRoute(route: MachineInstance[], fluidId: FluidId, amount: number) {
  if (amount <= 0) return
  for (const pipe of route) {
    spendTickBudget(activeFluidTransferBudgets, `fluid-segment:${pipe.uid}`, amount)
    if (!activeFluidSegmentFlows) continue
    const flows = activeFluidSegmentFlows.get(pipe.uid) ?? {}
    flows[fluidId] = (flows[fluidId] ?? 0) + amount
    activeFluidSegmentFlows.set(pipe.uid, flows)
  }
}

function steamRouteAllowanceMs(route: MachineInstance[], requestedMs: number, elapsedMs: number) {
  return route.reduce((allowanceMs, pipe) => {
    const capacityLitres = (machinePipeTransferLitresPerSecond(pipe.machineId) * elapsedMs) / 1000
    const availableLitres = consumeTickBudget(activeFluidTransferBudgets, `fluid-segment:${pipe.uid}`, capacityLitres, capacityLitres)
    return Math.min(allowanceMs, availableLitres * steamMsPerLitre)
  }, Math.max(0, requestedMs))
}

function spendSteamRoute(route: MachineInstance[], amountMs: number) {
  if (amountMs <= 0) return
  for (const pipe of route) {
    spendTickBudget(activeFluidTransferBudgets, `fluid-segment:${pipe.uid}`, amountMs / steamMsPerLitre)
    if (activeSteamSegmentFlows) activeSteamSegmentFlows.set(pipe.uid, (activeSteamSegmentFlows.get(pipe.uid) ?? 0) + amountMs)
  }
}

function recordFluidSourceFlow(source: MachineInstance, fluidId: FluidId, amount: number) {
  if (!activeFluidSourceFlows || amount <= 0) return
  const flows = activeFluidSourceFlows.get(source.uid) ?? {}
  flows[fluidId] = (flows[fluidId] ?? 0) + amount
...
export type SteamNetworkMetrics = {
  networkSize: number
  generationLitresPerSecond: number
  demandLitresPerSecond: number
  netLitresPerSecond: number
  storedLitres: number
...
export function steamNetworkMetrics(state: GameState, instance: MachineInstance): SteamNetworkMetrics {
  ...
  const generationLitresPerSecond = network.reduce((sum, candidate) => {
    if (candidate.machineId === 'steamBoiler' && candidate.process.activeRecipeId === 'make_steam') {
      return sum + boilerSteamProductionLitresPerSecond
    }
    if (isLiquidSteamBoilerMachine(candidate.machineId) && candidate.process.activeRecipeId === 'burn_creosote_steam') {
      return sum + liquidSteamBoilerSteamProductionLitresPerSecond
    }
    return sum
  }, 0)
  const demandLitresPerSecond = network.reduce((sum, candidate) => {
    if (candidate.machineId === 'steamTurbine' && candidate.process.activeRecipeId === 'generate_lv_eu') {
      return sum + steamTurbineSteamUseLitresPerSecond
    }
    if (candidate.machineId === 'steamAutoMiner' && candidate.process.activeRecipeId?.startsWith('auto_mine_')) {
      return sum + steamAutoMinerSteamUseLitres / (steamAutoMinerActionMs / 1000)
    }
    if (!isSteamPoweredMachine(candidate.machineId) || !candidate.process.activeRecipeId) return sum
    const recipe = processRecipes.find((entry) => entry.id === candidate.process.activeRecipeId)
    if (!recipe?.steamCostLitres || recipe.durationMs <= 0) return sum
    return sum + recipe.steamCostLitres / (recipe.durationMs / 1000)
  }, 0)
  const storedLitres = storage.reduce((sum, candidate) => sum + candidate.process.steamStoredMs / steamMsPerLitre, 0)
  const capacityLitres = storage.reduce((sum, candidate) => {
    if (isTankStorageMachine(candidate.machineId)) {
      return sum + steamTankCapacityMsForInstance(state, candidate) / steamMsPerLitre
    }
    return sum + candidate.process.steamCapacityMs / steamMsPerLitre
  }, 0)
  const netLitresPerSecond = generationLitresPerSecond - demandLitresPerSecond
  return {
    networkSize: network.length,
    generationLitresPerSecond,
    demandLitresPerSecond,
    netLitresPerSecond,
    storedLitres,
    capacityLitres,
    reserveSeconds: netLitresPerSecond < 0 ? storedLitres / Math.abs(netLitresPerSecond) : null,
  }
}
...
function fluidExportTransferRateLitres(state: GameState, source: MachineInstance, fluidId: FluidId) {
  ...
  const routeLimits = connectedFluidOutputTargets(state, source, fluidId)
    .map((target) => fluidRouteBetween(state, source, target))
    .filter((route): route is MachineInstance[] => Boolean(route))
    .map((route) => route.length > 0 ? Math.min(...route.map((pipe) => machinePipeTransferLitresPerSecond(pipe.machineId))) : sourceLimit)
  return routeLimits.length > 0 ? Math.min(sourceLimit, Math.max(...routeLimits)) : 0
}

function fluidExportFairRateLitres(state: GameState, source: MachineInstance, fluidId: FluidId) {
  const sourceCount = fluidExportSourceCountForNetwork(state, source, fluidId)
  return sourceCount > 1
    ? fluidExportTransferRateLitres(state, source, fluidId) / sourceCount
...
export function currentFluidOutputFlows(state: GameState, instance: MachineInstance): CurrentFluidOutputFlow[] {
  ...
  if (isSteamPipeMachine(instance.machineId)) {
    if (typeof instance.process.fluidFlowLitresPerSecond === 'number') {
      const fluidId = instance.process.fluidFlowFluidId
      if (!fluidId) return []
      return [{
        fluidId,
        litresPerSecond: instance.process.fluidFlowLitresPerSecond,
        storedLitres: instance.process.fluids[fluidId] ?? 0,
        freeLitres: Math.max(0, instance.process.fluidCapacityLitres - (instance.process.fluids[fluidId] ?? 0)),
      }]
    }
...
export function currentSteamPipeFlowLitresPerSecond(state: GameState, instance: MachineInstance) {
  ...
  if (typeof instance.process.steamFlowLitresPerSecond === 'number') return instance.process.steamFlowLitresPerSecond
...
export function currentEuCableFlowEuPerSecond(state: GameState, instance: MachineInstance) {
  ...
  if (typeof instance.process.euFlowPerSecond === 'number') return instance.process.euFlowPerSecond
  ...
  for (const instance of state.machineInstances) {
    if (!isSteamPipeMachine(instance.machineId) && canExportFluidSource(state, instance)) {
      ...
      const primaryFluidFlow = (Object.entries(measuredFlows) as Array<[FluidId, number]>)
        .sort((a, b) => b[1] - a[1])[0]
      instance.process.fluidFlowFluidId = primaryFluidFlow?.[0]
      instance.process.fluidFlowLitresPerSecond = Object.values(measuredFlows).reduce((sum, amount) => sum + (amount ?? 0), 0) / elapsedSeconds
    }
    if (isSteamPipeMachine(instance.machineId)) {
      ...
      const steamFlowLitres = ((activeSteamSegmentFlows?.get(instance.uid) ?? 0) / steamMsPerLitre) / elapsedSeconds
      const measuredFlows = activeFluidSegmentFlows?.get(instance.uid) ?? {}
      const primaryFluidFlow = (Object.entries(measuredFlows) as Array<[FluidId, number]>)
      ...
      const flowEu = (activeEuSegmentFlows?.get(instance.uid) ?? 0) / elapsedSeconds
      const availableEu = availableConnectedEu(state, instance)
...
function pushFluidToConnectedStorage(state: GameState, source: MachineInstance, fluidId: FluidId, elapsedMs: number) {
  ...
  const sourceBudgetKey = `fluid-source:${source.uid}:${fluidId}`
  const sourceTransferLimit = Math.max(0, Math.floor((fluidExportFairRateLitres(state, source, fluidId) * elapsedMs) / 1000))
  ...
  for (const [index, { storage, route }] of storageRoutes.entries()) {
    ...
    spendFluidRoute(route, fluidId, transfer)
    remaining -= transfer
    moved += transfer
  }
...
function consumeConnectedSteam(state: GameState, instance: MachineInstance, amount: number) {
  ...
  for (const storage of connectedSteamStorage(state, instance).sort((a, b) => a.uid.localeCompare(b.uid))) {
    if (remaining < 1) break
    const route = fluidRouteBetween(state, storage, instance)
    if (!route) continue
    const sourceBudgetKey = `steam-source:${storage.uid}`
    const sourceAvailable = consumeTickBudget(activeSteamTransferBudgets, sourceBudgetKey, storage.process.steamStoredMs, storage.process.steamStoredMs)
    const spend = Math.min(
      remaining,
      storage.process.steamStoredMs,
      sourceAvailable,
      steamRouteAllowanceMs(route, remaining, activeTransferElapsedMs),
    )
    if (spend < 1) continue
...
function steamTransferAllowanceMs(state: GameState, instance: MachineInstance, elapsedMs: number) {
  ...
  const routeRates = connectedSteamStorage(state, instance)
    .map((storage) => fluidRouteBetween(state, storage, instance))
    .filter((route): route is MachineInstance[] => Boolean(route))
    .map((route) => {
      const litresPerSecond = route.length > 0
        ? Math.min(...route.map((pipe) => machinePipeTransferLitresPerSecond(pipe.machineId)))
        : 24
      return litresPerSecond * steamMsPerLitre
    })
  const transferRateMs = routeRates.length > 0 ? Math.max(...routeRates) : connectedSteamTransferRateMs(state, instance)
  return Math.max(0, Math.floor((transferRateMs * elapsedMs) / 1000))
}
...
export function placeMachineInstance(state: GameState, machineId: MachineId, x: number, y: number) {
  ...
  if (occupied) {
    ...
    if (machineId === 'fabricationCable' && !hasFabricationCable(occupied) && isConductorMachine(occupied.machineId)) {
      const next = cloneState(state)
      next.machineInstances.find((instance) => instance.uid === occupied.uid)!.fabricationCableInstalled = true
      next.lastSavedAt = Date.now()
      return next
    }
    if (occupied.machineId === 'fabricationCable' && (machineId === 'itemConductor' || machineId === 'fluidConductor')) {
      const next = cloneState(state)
      const target = next.machineInstances.find((instance) => instance.uid === occupied.uid)!
      target.machineId = machineId
      target.fabricationCableInstalled = true
      next.lastSavedAt = Date.now()
      return next
    }
...
export function removeConductorLane(state: GameState, uid: string, lane: 'item' | 'fluid' | 'fabrication') {
  const instance = state.machineInstances.find((candidate) => candidate.uid === uid)
  if (!instance) return state
  const valid = lane === 'item'
    ? isItemConductorMachine(instance.machineId)
    : lane === 'fluid'
      ? isFluidConductorMachine(instance.machineId)
      : hasFabricationCable(instance)
  ...
  if (lane === 'fabrication') {
    ...
    for (const card of next.recipeCards) {
      if (card.installedInUid && activeInterfaceUids.has(card.installedInUid)) delete card.installedInUid
    }
    target.fabricationCableInstalled = false
    target.fabricationInterfaces = {}
    next.lastSavedAt = Date.now()
    return next
  }
  if (instance.machineId !== 'conductorBundle' && hasFabricationCable(instance)) {
    const next = cloneState(state)
    const target = next.machineInstances.find((candidate) => candidate.uid === uid)!
    target.machineId = 'fabricationCable'
    target.fabricationCableInstalled = true
    if (lane === 'item') {
...
function pullFluidFromConnectedSources(state: GameState, instance: MachineInstance, fluidId: FluidId, amount: number, elapsedMs: number) {
  ...
  for (const source of connectedFluidSources(state, instance, fluidId)) {
    if (remaining < 1) break
    const route = fluidRouteBetween(state, source, instance)
    if (!route) continue
    const stored = source.process.fluids[fluidId] ?? 0
    const fullSourceLimit = Math.max(0, Math.floor((fluidExportFairRateLitres(state, source, fluidId) * elapsedMs) / 1000))
    const sourceBudgetKey = `fluid-source:${source.uid}:${fluidId}`
    const sourceLimit = consumeTickBudget(activeFluidTransferBudgets, sourceBudgetKey, fullSourceLimit, fullSourceLimit)
    const transfer = Math.min(remaining, stored, sourceLimit, fluidRouteAllowance(route, remaining, elapsedMs))
    if (transfer < 1) continue
    source.process.fluids[fluidId] = stored - transfer
    spendTickBudget(activeFluidTransferBudgets, sourceBudgetKey, transfer)
    spendFluidRoute(route, fluidId, transfer)
    recordFluidSourceFlow(source, fluidId, transfer)
    remaining -= transfer
    moved += transfer
  }
  return moved
...
export function removeProcessSlot(state: GameState, uid: string, slotId: ProcessSlotId) {
  ...
  if (outputIndex >= 0 && instance && !isItemHopperMachine(instance.machineId)) return collectProcessOutput(state, uid, outputIndex as 0 | 1)
...
export function collectProcessOutput(state: GameState, uid: string, outputIndex: 0 | 1 = 0) {
  const instance = state.machineInstances.find((candidate) => candidate.uid === uid)
  const slotId = processOutputSlotIds[outputIndex]
  const owner = instance ? arcProcessSlotOwner(state, instance, slotId) : null
...
function tickCokeOven(state: GameState, instance: MachineInstance, elapsedMs: number) {
  ...
  while (remainingMs > 0) {
    const recipe = findProcessRecipeForInput(instance.machineId, process.input)
    if (!recipe?.output || !canOutputAccept(process.output, recipe.output) || !canFluidOutputAccept(state, instance, recipe)) {
      process.activeRecipeId = null
      if (!recipe) {
        process.progressMs = 0
        process.durationMs = 0
...
function hopperPullCandidate(state: GameState, instance: MachineInstance, sourceDirections: PipeDirection[]): HopperPullCandidate | null {
  for (const direction of sourceDirections) {
    ...
    for (const sourceSlotId of processOutputSlotIds) {
...
function tickLvItemAutomation(state: GameState, source: MachineInstance, elapsedMs: number) {
  ...
  if (!processOutputSlotIds.some((slotId) => source.process[slotId])) {
    source.itemTransferProgressMs = 0
    return
  }
  ...
  if (!processOutputSlotIds.some((slotId) => source.process[slotId])) source.itemTransferProgressMs = 0
}
...
function removableInventorySlots(instance: MachineInstance) {
  ...
  if (isItemHopperMachine(instance.machineId)) {
    ...
    for (const slotId of processOutputSlotIds.slice(0, outputCount)) {
      slots.push({ get: () => instance.process[slotId], set: (slot) => { instance.process[slotId] = slot } })
    }
  }
  return slots
}
...
function insertOneIntoInventory(instance: MachineInstance, resourceId: ResourceId) {
  ...
  const candidateSlots: ProcessSlotId[] = isItemHopperMachine(instance.machineId)
    ...
    : ['input', 'secondaryInput', ...mixerExtraInputSlotIds, 'fuel']
...
function processSlotsAreEmpty(process: MachineProcessState) {
  return !process.input &&
    ...
    !mixerExtraInputSlotIds.some((slotId) => process[slotId]) &&
    !process.fuel &&
    !processOutputSlotIds.some((slotId) => process[slotId]) &&
...
function completeFabricationJob(state: GameState, job: FabricationJob) {
  const network = job.controllerUid ? fabricationNetworkForController(state, job.controllerUid) : null
  if (job.reservedFluids.length > 0 && !network) {
    job.status = 'blocked'
    job.blockedReason = 'Fabrication network is disconnected'
    return
  }
  for (const fluid of job.reservedFluids) {
    if (returnLinkedFluid(state, network!, fluid.id, fluid.amount) > 0) {
      job.status = 'blocked'
      job.blockedReason = `No linked tank capacity for ${fluidLabels[fluid.id]}`
      return
    }
  }
  const returnedItems = job.reservedItems.map((amount) => ({ ...amount }))
  for (const amount of returnedItems) state.resources[amount.id] += amount.amount
  recordResourceMilestones(state, returnedItems)
  job.reservedItems = []
  job.reservedFluids = []
  job.status = 'complete'
  job.blockedReason = undefined
  state.recipeMilestones.fabrication_job_complete = (state.recipeMilestones.fabrication_job_complete ?? 0) + 1
}
...
function tickFabricationJobs(state: GameState, elapsedMs: number) {
  ...
  for (const job of activeJobs) {
    ...
    if (!rack || !network) {
      job.status = 'blocked'
      job.blockedReason = rack ? 'Fabrication cable network is disconnected' : 'Planning rack is incomplete'
      continue
    }
    const activeStep = job.steps.find((step) => step.completedBatches < step.batches)
    if (!activeStep) {
      completeFabricationJob(state, job)
      continue
    }
    ...
    const controllerUse = Math.min(rack.controller.process.euStored, (8 * elapsedMs) / 1000)
    rack.controller.process.euStored -= controllerUse
    job.status = 'running'
    job.blockedReason = undefined

    if (card.kind === 'crafting') {
      if (target.machineId !== 'autoFabricator') {
        job.status = 'blocked'
        job.blockedReason = 'Crafting card requires an adjacent Auto Fabricator'
        continue
      }
      if (!activeStep.dispatched) {
        if (!removeReservedItems(job, card.itemInputs)) {
          job.status = 'blocked'
          job.blockedReason = 'Reserved crafting inputs are incomplete'
          continue
        }
        activeStep.dispatched = true
        job.progressMs = 0
      }
      if (rack.controller.process.euStored < 32) {
        job.status = 'blocked'
        job.blockedReason = 'Planning Controller needs 32 EU for the Auto Fabricator'
        continue
      }
      job.progressMs += elapsedMs
      if (job.progressMs < 2000) continue
      rack.controller.process.euStored -= 32
      addReservedItems(job, card.itemOutputs)
      addReservedFluids(job, card.fluidOutputs)
      activeStep.completedBatches += 1
      job.completedBatches += 1
      activeStep.dispatched = false
      job.progressMs = 0
      continue
    }
    ...
    if (!activeStep.dispatched) {
      ...
      const fluidTransferMs = (recipeFluidInputs(recipe).reduce((sum, fluid) => sum + fluid.amount, 0) / 64) * 1000
      if (fluidTransferMs > 0) {
        job.progressMs += elapsedMs
...
function tickMachineInstancesInPlace(next: GameState, elapsedMs: number, now = Date.now(), topology = activateFactoryTopology(next)) {
  ...
  for (const instance of next.machineInstances) {
    if (isTankStorageMachine(instance.machineId)) fillSteamTankFromConnectedStorage(next, instance, elapsedMs)
  }
  for (const instance of next.machineInstances) {
...
function questObjectiveCurrent(state: GameState, objective: QuestObjective) {
  ...
  if (objective.type === 'fabrication') {
    const milestoneId = objective.id === 'cardEncoded'
      ...
      : objective.id === 'rackFormed'
        ? 'fabrication_rack_formed'
        : 'fabrication_job_complete'
    return state.recipeMilestones[milestoneId] ?? 0
  }
  ...
  if (objective.type === 'installedBattery') {
    return state.machineInstances
      .filter((instance) => instance.machineId === objective.id)
      .reduce((sum, instance) => sum + batteryBufferInstalledBatteries(instance), 0)
  }
...
function migrateMachines(parsedMachines?: Partial<Record<string, number>>): Record<MachineId, number> {
  ...
  const legacyRackFrames = parsedMachines?.rackFrame
  if (typeof legacyRackFrames === 'number') migrated.memoryModule += Math.max(0, Math.floor(legacyRackFrames))
  return migrated
}
...
function migrateMachineInstances(
  ...
  machinesState.fabricationCable = Math.max(
